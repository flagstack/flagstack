...
import type { CreateEnvironmentPayload, Environment } from '../../environment/types'
...
export function CreateEnvironmentForm({ organisation, projectID, onCancel, onCreated }: CreateEnvironmentFormProps) {
  const [form, setForm] = useState<CreateEnvironmentPayload>({ name: '', key: '', description: '' })
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const environment = await apiRequest<Environment>(
        `/api/v1/organisations/${encodeURIComponent(organisation.slug)}/projects/${encodeURIComponent(projectID)}/environments`,
        { method: 'POST', body: JSON.stringify(form) },
      )
      onCreated(environment)
    } catch (requestError) {
      setError(requestError instanceof APIError ? requestError.message : 'Environment could not be created.')
    } finally {
      setSubmitting(false)
    }
  }
        ...
        <label className="text-xs font-medium text-slate-300">
          ...
          <input
            ...
            onChange={(event) => {
              ...
              setForm((current) => ({ ...current, name, key: keyEdited ? current.key : environmentKey(name) }))
            }}
